...
void MySolver::compute_F_grad_hess()
{
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
  {
    ...
    for (unsigned int q = 0; q < n_q_points; ++q)
    {
      ...
      E_h = (
        c11/4*r*pow(sg_prime,4) + 
        c11*r*pow(sg_prime,3) +
        c11*r*pow(sg_prime,2) +
        c12*sg*pow(sg_prime,2) +
        2*c12*sg*sg_prime +
        c12/2/r*pow(sg,2)*pow(sg_prime,2) +
        c12/r*pow(sg,2)*sg_prime +
        c22/r*pow(sg,2) +
        c22/pow(r,2)*pow(sg,3) +
        c22/4/pow(r,3)*pow(sg,4)
      ) * fe_values.JxW(q);
  ...
  grad_F[n_dofs-1] += 2*radius*pressure*(2/radius*solution[n_dofs-1] + 2);
  //hess_F[n_dofs-1][n_dofs-1] += 2 * c12 * radius;
  hess_F[n_dofs-1][n_dofs-1] += 4*pressure;
...
void MySolver::compute_alpha_derivs(double alpha, double &dF_dAlpha, double &d2F_dAlpha2)
{
  ...
  //      c11/2 * E_h;
  //E_h = E_h + pressure*(new_s.back()/radius + 1);
  E_h = E_h + 2*radius*pressure*(new_s.back()/radius+1)*(new_s.back()+radius);
  F_delta = E_h;
